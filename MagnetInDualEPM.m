...
EPM2_Diretion = [0, -1, 0];       % Magnetic moment vector of the first dipole (aligned along z-axis)
...
% Dipole Positions
EPM_Pos = [-0.3, 0, 0];            % Position of the first dipole
EPM2_Pos = [0, 0.3, 0];            % Position of the first dipole
...
% Calculate field components for the EPM1 
x1 = x - EPM_Pos(1);
y1 = y - EPM_Pos(2);
z1 = z - EPM_Pos(3);
...
% Calculate field components for the EPM2
x3 = x - EPM2_Pos(1);
y3 = y - EPM2_Pos(2);
z3 = z - EPM2_Pos(3);
...
Bx3 = mu0/(4*pi) * (3*(m2_EPM(1)*rx3 + m2_EPM(2)*ry3 + m2_EPM(3)*rz3).*rx3 - m2_EPM(1))./r3.^3;
By3 = mu0/(4*pi) * (3*(m2_EPM(1)*rx3 + m2_EPM(2)*ry3 + m2_EPM(3)*rz3).*ry3 - m2_EPM(2))./r3.^3;
Bz3 = mu0/(4*pi) * (3*(m2_EPM(1)*rx3 + m2_EPM(2)*ry3 + m2_EPM(3)*rz3).*rz3 - m2_EPM(3))./r3.^3;
...
Bx_total = Bx1 + Bx3;%+ Bx2;                            
By_total = By1 + By3;%+ By2;
Bz_total = Bz1 + Bz3;%+ Bz2;
...
[idx3, idy3, idz3] = findClosestGridPoint(x, y, z, EPM2_Pos);

%% Compute Torques on Dipole
T11 = f_getTorque(Bx1, By1, Bz1, idx2, idy2, idz2, m_dipole);
T21 = f_getTorque(Bx3, By3, Bz3, idx2, idy2, idz2, m_dipole);
...
[decX,decY,decZ] = decimateVectors(Bx_total,By_total,Bz_total,8);
...
%plot3(EPM2_Pos(1), EPM2_Pos(2), EPM2_Pos(3), 'ro', 'MarkerSize', 3, 'MarkerFaceColor', 'b'); % First dipole position
